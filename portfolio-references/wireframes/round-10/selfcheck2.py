...
const proj='[data-block="SB1"],[data-block="AD1"],[data-block="PS"],[data-block="PSD1"],[data-block="PS7"],[data-block="PSD7_1"],[data-block^="RF1"],[data-block="RCar"],[data-block^="RP1"],[data-block^="PF1"],[data-block^="IPF1"],[data-block^="RC1"],[data-block^="P1"],[data-block^="FP1"],[data-block="GI"],[data-block="HT1"],[data-block^="IL"],[data-block^="IP1"],[data-block="JP1"],[data-block^="LP1"]';
const pd=[...document.querySelectorAll(proj)].filter(vis);
let decY=1e9; for(const b of pd){ for(const e of b.querySelectorAll("dt,th,h3,li,summary,p,span")){ if(/Decision|The choice|Row locking/.test(e.textContent)&&vis(e)){decY=Math.min(decY,y(e));break;} } }
...
let cpl=null; const dt=[...document.querySelectorAll('dt')].find(e=>(e.textContent=="Problem"||e.textContent=="The problem")&&vis(e));
...
else{const td=[...document.querySelectorAll('th')].find(e=>(e.textContent=="Problem"||e.textContent=="The problem"));if(td){const c=td.nextElementSibling;const lh=parseFloat(getComputedStyle(c).lineHeight)||24;cpl=Math.round(c.textContent.length/Math.round((c.getBoundingClientRect().height-16)/lh));}}
const small=[...document.querySelectorAll('nav a, summary, [data-block="HB"] a, [data-block="PS"] a, [data-block="HB7"] a, [data-block="HP7"] a, [data-block="PS7"] a, [data-block="KCm"] button, [data-block="HBs"] button, [data-block^="C1"] a, [data-block^="IL"] a, [data-block^="IP"] > a')].filter(vis).map(e=>{const b=e.getBoundingClientRect();return {t:e.textContent.slice(0,24),w:Math.round(b.width),h:Math.round(b.height)}}).filter(o=>o.w<44||o.h<44);
